...
export function getPreviousAndNextDates(targetDay: number, start?: string) {
    ...
    if (currentDay === targetDay) {
        ...
        if (now.getUTCHours() >= 11) {
            ...
            startDate = new Date(Date.UTC(now.getUTCFullYear(), now.getUTCMonth(), now.getUTCDate(), 11, 0, 0));
            ...
            endDate.setUTCHours(10,59,59, 999); // Ensure it's set to 17:59:59.999
            ...
            startDate = new Date(Date.UTC(now.getUTCFullYear(), now.getUTCMonth(), now.getUTCDate() - 7, 11, 0, 0));
            console.log("Start Date set to previous week:", startDate.toISOString());

            // Set end date to today at 17:59:59.999 UTC
            endDate = new Date(Date.UTC(now.getUTCFullYear(), now.getUTCMonth(), now.getUTCDate(), 10, 59, 59, 999));
        ...
        startDate = new Date(Date.UTC(previousTargetDate.getUTCFullYear(), previousTargetDate.getUTCMonth(), previousTargetDate.getUTCDate(), 11, 0, 0));
