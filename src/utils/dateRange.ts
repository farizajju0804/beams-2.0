...
export function getPreviousAndNextDates(targetDay: number, start?: string) {
    ...
    if (currentDay === targetDay) {
        ...
        if (now.getUTCHours() >= 18) {
            ...
            endDate.setUTCHours(13,36, 59, 999); // Ensure it's set to 17:59:59.999
            ...
            endDate = new Date(Date.UTC(now.getUTCFullYear(), now.getUTCMonth(), now.getUTCDate(), 13, 36, 59, 999));
