...
import { AddCircle, ArrowDown2, MinusCirlce } from 'iconsax-react';
...
const SideBar: React.FC = () => {
  ...
  const renderIcon = (completed: boolean) => {
    if (completed) {
      return <FaCircleCheck size={22} className='text-brand shrink-0' />;
    } else {
  ...
  return (
    <div className={`hidden lg:block transition-width duration-300 ${isOpen ? 'w-auto' : 'w-fit'}`}>
        ...
        {sections.map((section, idx) => (
          <div key={idx} className='my-4'>
            {section.items.map((item, index) => {
              const allSubitemsCompleted = item.subitems?.every(subitem => subitem.completed);
                ...
                <div key={index} className='p-2'>
                  <div className='flex justify-between items-center cursor-pointer' onClick={() => toggleItem(index)}>
                    ...
                    <div className='flex items-center justify-center rounded-full p-2'>
                      <motion.div
                        initial={false}
                        animate={{ rotate: allSubitemsCompleted ? 0 : expandedItems.has(index) ? 180 : 0 }}
